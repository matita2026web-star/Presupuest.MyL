...
import { 
  ...
  Settings as SettingsIcon, Menu, Wifi, WifiOff, 
  Briefcase, Activity, PlusCircle, DownloadCloud, ShieldCheck,
  HardHat, ChevronRight, Database, User, Loader2
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = () => {
      try {
        ...
        addNotification("Error cargando datos locales", "info");
      } finally {
  ...
  const handleSaveBudget = (newBudget: Budget) => {
    ...
    if (exists !== -1) {
      ...
      addNotification("Nuevo presupuesto creado", "success");
    }
    
    setBudgets(updatedBudgets);
    storage.saveBudgets(updatedBudgets); // Guarda el array completo en LocalStorage
  ...
  const handleDeleteBudget = (id: string) => {
    ...
    addNotification("Registro eliminado del archivo", "info");
  };
  ...
  const handleUpdateSettings = (newSettings: BusinessSettings) => {
    ...
    addNotification("Configuración de empresa guardada");
  };
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <h2 className="text-3xl font-black uppercase italic tracking-tighter">
              Cargando <span className="text-orange-500">Sistema</span>
            </h2>
          </div>
  ...
  return (
    <div className="min-h-screen flex bg-[#f1f5f9] text-slate-900 overflow-hidden font-sans selection:bg-orange-100">
      ...
      <main className="flex-1 flex flex-col min-w-0 h-screen relative">
        ...
        <header className="h-24 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between px-6 sm:px-10 sticky top-0 z-30 shadow-sm">
          ...
          <div className="flex items-center gap-8">
              ...
              <div className="hidden md:flex items-center bg-slate-100 p-1.5 rounded-2xl border border-slate-200">
                 <button onClick={() => { setEditingBudget(null); setActiveTab('generator'); }} className="p-3 hover:bg-white rounded-xl text-slate-500 hover:text-orange-500 transition-all" title="Nuevo Presupuesto">
                    <PlusCircle size={20} />
                 </button>
                 <button onClick={() => addNotification("Base de datos local verificada", "info")} className="p-3 hover:bg-white rounded-xl text-slate-500 hover:text-orange-500 transition-all" title="Verificar DB">
                    <Database size={20} />
                 </button>
              </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-[#f8fafc]">
          <div className="max-w-7xl mx-auto p-4 sm:p-8 md:p-12 pb-32">
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              {activeTab === 'dashboard' && (
                <Dashboard 
                  ...
                  settings={settings} 
                  onNavigate={(tab) => {
                    if (tab === 'generator') setEditingBudget(null);
                    setActiveTab(tab);
                  }}
              ...
              {activeTab === 'generator' && (
                <BudgetGenerator 
                  ...
                  initialBudget={editingBudget} // Corrección clave: initialData -> initialBudget
                  onCancel={() => {
                    setEditingBudget(null);
                    setActiveTab('history');
                  }}
          ...
          {/* FOOTER */}
          <footer className="max-w-7xl mx-auto px-12 py-6 opacity-30 flex justify-between items-center text-[10px] font-black uppercase tracking-[0.3em] border-t border-slate-200/50">
             <div className="flex items-center gap-4">
             ...
             <div className="flex items-center gap-2">
                <DownloadCloud size={12}/> Local Storage Active
             </div>
          </footer>
        </div>
      </main>
