...
const BudgetHistory: React.FC<BudgetHistoryProps> = ({ 
  ...
  const stats = useMemo(() => {
    return {
      totalAmount: filtered.reduce((acc, b) => acc + (b.total || 0), 0),
      count: filtered.length,
      pendingCount: filtered.filter(b => b.status === 'pendiente').length,
      acceptedAmount: filtered.filter(b => b.status === 'aceptado').reduce((acc, b) => acc + (b.total || 0), 0)
    };
  }, [filtered]);
  ...
  const handleExportPDF = (budget: Budget) => {
    ...
    doc.text((settings?.name || 'CONSTRUCTORA').toUpperCase(), 20, 25);
  ...
  return (
    <div className="space-y-6 animate-slide-up pb-32">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div onClick={() => { setSearchTerm(''); setStatusFilter('todos'); }} className="bg-white p-6 rounded-[2rem] border-2 border-slate-100 shadow-sm flex flex-col justify-center items-center group cursor-pointer hover:border-amber-500 transition-all">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {filtered.map(b => (
          <div key={b.id} className="group bg-white rounded-[3rem] border-2 border-slate-100 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 flex flex-col relative overflow-hidden">
            ...
            <div className="p-10 flex-1 relative z-10">
              <div className="flex justify-between items-start mb-8">
                <div onClick={() => copyToClipboard(b.id)} className="flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-xl cursor-pointer hover:bg-slate-100 transition-all border border-slate-100">
                ...
                <div className="flex gap-1 items-center">
                  <button onClick={() => onUpdateStatus(b.id, 'aceptado')} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-emerald-50 text-slate-300 hover:text-emerald-500 transition-all">
                    <CheckCircle size={18} />
                  </button>
              ...
              <div className="grid grid-cols-2 gap-3 mb-10">
                <button onClick={() => handleExportPDF(b)} className="flex items-center justify-center gap-3 py-4 bg-slate-50 text-slate-900 rounded-2xl text-[10px] font-black uppercase hover:bg-slate-900 hover:text-white transition-all border border-slate-100 group/btn">
                  <Download size={16} className="text-amber-500 group-hover/btn:text-white" /> EXPORTAR PDF
                </button>
                <button onClick={() => handleWhatsApp(b)} className="flex items-center justify-center gap-3 py-4 bg-slate-50 text-slate-900 rounded-2xl text-[10px] font-black uppercase hover:bg-emerald-500 hover:text-white transition-all border border-slate-100 group/btn">
      ...
      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-32 bg-white rounded-[4rem] border-4 border-dashed border-slate-100">
          ...
          <button onClick={() => { setSearchTerm(''); setStatusFilter('todos'); }} className="mt-10 px-10 py-4 bg-slate-900 text-white rounded-2xl font-black uppercase italic text-xs hover:bg-amber-500 hover:text-slate-900 transition-all shadow-xl">
            RESETEAR ARCHIVO
          </button>
        </div>
      )}
         ...
         <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <div>
               ...
               <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-md">
                 Sistema de gestión técnica optimizado para la trazabilidad de obras.
               </p>
            </div>
            <div className="bg-white/5 backdrop-blur-md rounded-3xl p-8 border border-white/10 grid grid-cols-2 gap-8">
               <div>
                  ...
                  <p className="text-4xl font-black text-white italic">{budgets?.length || 0}</p>
               </div>
               <div>
